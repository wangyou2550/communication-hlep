...
class PersonCenter(QWidget):
    ...
    def on_node_clicked(self, item, column):
        ...
        if item.childCount()==0:
            ...
            stepCenter=StepCenter(item.id)
            stepCenter.add_dialog_signal.connect(self.show_dialog_in_table_widget)
            # # 小节
            # section=Section(item.id)
            # # dialog显示的信号槽
            # section.add_dialog_signal.connect(self.show_dialog_in_table_widget)
            self.tab_widget.addTab(stepCenter,item.name)
            # 设置为当前页面
            self.tab_widget.setCurrentWidget(stepCenter)

    def creatMenu(self, menuData):
        menu = QTreeWidget(self)
        # 添加树节点
        menu.headerItem().setText(0, "个人中心")
        # 树节点点击事件
        ...
        for chapter in menuData:
            ...
            self.showSection(chapter["node"],chapter_item)
    ...
    def initUI(self):
        ...
        self.tab_widget.addTab(widget2,"个人中心")
