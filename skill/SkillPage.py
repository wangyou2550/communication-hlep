...
class SkillPage(QWidget):
    ...
    def initUI(self):
        ...
        self.skillWidget=KnowledgeSkillManager()
        self.skillWidget.add_widget_signal.connect(self.show_widget_in_table_widget)
        self.tab_widget.addTab(self.skillWidget, "技巧")
    ...
    def on_node_clicked(self, item, column):
        # 根据id获取知识点的内容
        print(item.id)
        skillWidget=KnowledgeSkillManager()
        ...
        self.tab_widget.addTab(skillWidget,item.name+'技巧')
        # 设置为当前页面
        self.tab_widget.setCurrentWidget(skillWidget)
    ...
    @pyqtSlot(QWidget)
    def show_widget_in_table_widget(self,widget):
        self.tab_widget.addTab(widget,"技巧")
        # 设置为当前页面
