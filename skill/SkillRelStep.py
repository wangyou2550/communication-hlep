...
class SkillRelStepDialog(QDialog):
    add_dialog_signal = pyqtSignal(QDialog)
    def __init__(self, skill_id, skill_name, parent=None):
        ...
        self.steps=HttpTool.get(PathConstant.GET_SKILL_RELATION_STEP  + str(skill_id))
        self.initUI()
        self.resize_dialog()
    ...
    def initUI(self):
        self.setWindowTitle(self.skill_name)
    ...
    def createImageStackWidget(self):
        ...
        if len(self.steps)>0:
            ...
            self.image_widget = ImageViewer(self.steps[0]["imageSrc"])
            # self.update_step_feedback_favorite(self.steps[0])
    ...
    def image_display(self,i):
        ...
        self.image_widget = ImageViewer(self.steps[i]["imageSrc"])
...
# 收藏反馈按钮
    ...
    def buttonClick(self):
        ...
        print(button_text)
        if button_text =="步骤新增":
            dialog=StepDialog(self.skill_id)
            dialog.exec_()
