...
class Section(QWidget):
    ...
    def create_operation_button(self):
        ...
        self.add_push_button.setText("新增")
...
# 创建知识点树形列表
    def createKnowledgeNodeTableWidget(self):
        ...
        for node in self.nodes:
            child = QTreeWidgetItem(self.node_table_widget, str(node["id"]),node["id"],node["pid"])
            checkbox = QCheckBox()
            checkbox.setCheckable(True)
            checkbox.setChecked(node["learn"])
            self.node_table_widget.setItemWidget(child, 1, checkbox)
            sort_lable=QLabel()
            sort_lable.setText(node["sort"])
